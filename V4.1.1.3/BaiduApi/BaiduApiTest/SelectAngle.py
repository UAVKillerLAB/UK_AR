...
def init():
    ...
    with open(r'C:\Users\ok\Desktop\pc_antiuav_allversion\4.1.1\BaiduApiTest_b\BaiduApiTest\config.json', 'r') as f:
        ...
        # print("主站纬度：" + str(station10_latitude))
        ...
        # print("一站纬度：" + str(station8_latitude))
        ...
        print("\n读取成功！\n")
    create_csv()
    # s10_correction_angle = input("请输入修正角度（天线90°基线与正北方向差（由北向西计算））：")
...
def write_csv(latitude, longitude):
    # path = "aa.csv"
    with open(csv_name, 'a+', newline='') as f:
        csv_write = csv.writer(f)
        data_row = [latitude, longitude]
...
def positioning(station10_angle, station8_angle, station10_correction, station8_correction, station10_latitude,
    ...
    temptime = time.strftime('%H:%M:%S', time.localtime(time.time()))  # 显示当前时间
    # write_csv(temptime, target_latitude, target_longitude)
    # # write_csv(temp, target_latitude, target_longitude)

    data_show = []

    data_show.append(target_longitude)  # 写入经度
    data_show.append(target_latitude)  # 写入纬度
    data_show.append(count)  # 往data_show里存入数据
    data_show.append(temptime)

    # with open('./rec_gps.csv', 'a', newline='') as csvFile1:
    #     writer = csv.writer(csvFile1)
    #     writer.writerow((count, temptime, target_longitude, target_latitude))
    #     csvFile1.close()
    return data_show
...
def main(request):
    global count, matched_angle_buff
    count += 1
    ...
    data_show = positioning(s10_matched_angle, s8_matched_angle, s10_correction_angle, s8_correction_angle, station10_latitude,
                            station10_longitude)
    jsontemp = json.dumps(data_show)

    temp_data_now = []
    temp_data_now.append(data_show)
    temp_data_now.append(decoded_data[0])
    temp_data_now.append(decoded_data[1])
    temp_data_now.append(decoded_data[2])
    temp_data_now.append(decoded_data[3])
    temp_data_now.append(s10_matched_angle)
    temp_data_now.append(s8_matched_angle)

    with open(csv_name_temp, 'a+', newline='') as f:
        csv_write = csv.writer(f)
        data_row = temp_data_now
        csv_write.writerow(data_row)
...
csv_name_temp = r"location_info/"
csv_name_temp = csv_name_temp + str(time.strftime("%H%M")) + "_temp.csv"

with open(csv_name_temp, 'w', newline='') as f:
    csv_write = csv.writer(f)
    csv_head = ["count", "time", "ch1", "ch2", "ch3", "ch4", "s10_matched_angle", "s8_matched_angle",
                "main_distance", "slove_distance"]
    csv_write.writerow(csv_head)
    print("创建成功！")