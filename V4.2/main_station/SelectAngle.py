...
def main():
    ...
    while True:
        if run_flag == 1:
            ...
            Print4(count, decoded_data[0], decoded_data[1], decoded_data[2], decoded_data[3], matched_angle_mid)
            print("当前角度为:{}".format(matched_angle_mid))
            """
            解算函数
            """
            s8_matched_angle = recv_mess[0]
            time10 = time.time()
            save = positioning(matched_angle_mid, s8_matched_angle, s10_correction_angle, s8_correction_angle, station10_latitude,
                        station10_longitude)
            print("\n\n")
